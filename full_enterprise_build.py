...
for project in projects:
    ...
    # ------------------------
    # 2. Create README.md per project
    # ------------------------
    readme_path = os.path.join(BASE_PATH, project, "README.md")
    with open(readme_path, "w") as f:
        f.write(f"# {project.replace('_', ' ').title()}\n")
        f.write(f"Professional data wrangling project: {project}\n\n")
    ...
    # ------------------------
    req_path = os.path.join(BASE_PATH, project, "requirements.txt")
...
print("✅ Project folders, main.py, README.md, requirements.txt, and sample datasets created.")
...
if not status:
    ...
    git(["add", "."])
    commit_msg = f"Enterprise build: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
